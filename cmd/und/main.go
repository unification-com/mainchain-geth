...
// und is the official command-line client for Ethereum.
package main
...
var (
	...
	app = utils.NewApp(gitCommit, gitDate, "the und command line interface")
...
func init() {
	...
	app.Before = func(ctx *cli.Context) error {
		...
		if ctx.GlobalString(utils.SyncModeFlag.Name) != "light" && !ctx.GlobalIsSet(utils.CacheFlag.Name) && !ctx.GlobalIsSet(utils.NetworkIdFlag.Name) {
			...
			if !ctx.GlobalIsSet(utils.TestnetFlag.Name) && !ctx.GlobalIsSet(utils.RinkebyFlag.Name) && !ctx.GlobalIsSet(utils.GoerliFlag.Name) && !ctx.GlobalIsSet(utils.DeveloperFlag.Name) && !ctx.GlobalIsSet(utils.UndDevNetFlag.Name) {
...
func startNode(ctx *cli.Context, stack *node.Node) {
	...
	// Create a client to interact with local und node.
