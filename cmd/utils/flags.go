...
import (
	...
	"github.com/unification-com/mainchain/graphql"
...
var (
	...
	UndDevNetFlag = cli.BoolFlag{
		Name: "devnet",
		Usage: "Preconfigured UND developer network",
	}
...
func setDataDir(ctx *cli.Context, cfg *node.Config) {
	...
	case ctx.GlobalBool(UndDevNetFlag.Name) && cfg.DataDir == node.DefaultDataDir():
		cfg.DataDir = filepath.Join(node.DefaultDataDir(), "unddevnet")
	}
}
...
func SetEthConfig(ctx *cli.Context, stack *node.Node, cfg *eth.Config) {
	...
	CheckExclusive(ctx, DeveloperFlag, TestnetFlag, RinkebyFlag, GoerliFlag, UndDevNetFlag)
	...
	case ctx.GlobalBool(UndDevNetFlag.Name):
		log.Info("und.devnet flag enabled. Load default UndDevNet Genesis")
		if !ctx.GlobalIsSet(NetworkIdFlag.Name) {
			cfg.NetworkId = 50009
		}
		cfg.Genesis = core.DefaultUndDevnetGenesisBlock()
	case ctx.GlobalBool(DeveloperFlag.Name):
		if !ctx.GlobalIsSet(NetworkIdFlag.Name) {
			cfg.NetworkId = 1337
		}
...
func SetupMetrics(ctx *cli.Context) {
	if metrics.Enabled {
		...
		if enableExport {
			...
			go influxdb.InfluxDBWithTags(metrics.DefaultRegistry, 10*time.Second, endpoint, database, username, password, "und.", tagsMap)
		}
	}
}
...
func MakeGenesis(ctx *cli.Context) *core.Genesis {
	...
	case ctx.GlobalBool(UndDevNetFlag.Name):
		genesis = core.DefaultUndDevnetGenesisBlock()
	case ctx.GlobalBool(DeveloperFlag.Name):
