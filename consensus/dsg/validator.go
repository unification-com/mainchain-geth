...
func (bp *BlockProposal) ValidateBlockProposal() bool {
	...
	if signerIndex != bp.ProposerId.Uint64() {
		log.Info("Invalid block proposal - signerIndex mismatch", "exp", signerIndex, "got", bp.ProposerId.Uint64())
		return false
	} else {
	...
	if txHash != txDerivedHash {
		log.Info("Invalid block proposal - txHash mismatch", "exp", txDerivedHash, "got", txHash)
		return false
	} else {
